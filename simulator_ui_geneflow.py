...
COLORS = {
    ...
    "surface2": "#28282d",
    "text-primary": "#d7d2c9",
    "text-secondary": "#8e877b",
    "text-tertiary": "#676158",
    "accent": "#d1ad6a",
    "teal": "#d1ad6a",
    "teal2": "#e2c47b",
    "accent-blue": "#d1ad6a",
...
LIGHT_COLORS = {
    ...
    "surface2": "#e7ddcf",
    "text-primary": "#2f271c",
    "text-secondary": "#5d4f3b",
    "text-tertiary": "#7d6f58",
    "accent": "#8a6a35",
    "teal": "#8a6a35",
    "teal2": "#9b7840",
    "accent-blue": "#8a6a35",
...
class FADiagramWidget(QWidget):
    ...
    def _compute_layout(self):
        ...
        if states == 1:
            self._radius = 24
            self._positions = {0: (width / 2, height / 2)}
            return

        margin = 24
        raw_step = (width - 2 * (margin + 18)) / max(1, states - 1)
        step = max(28, min(120, raw_step))
        self._radius = max(14, min(24, int(step * 0.26)))

        start_x = margin + self._radius
        max_x = width - margin - self._radius
        if states > 1:
            step = max(16, (max_x - start_x) / (states - 1))

        y = max(112, height * 0.56)
    ...
    def _draw_nodes(self, painter: QPainter):
        ...
        for state, (x, y) in self._positions.items():
            if state == self._accept_state:
                ...
                label_color = QColor(COLORS["success"])
            else:
                fill = QColor(COLORS["panel"])
                pen = QPen(QColor(COLORS["border"]), 3)
                palette = [COLORS["accent"], COLORS["gold"], COLORS["green"], COLORS["violet"], COLORS["text-primary"]]
                label_color = QColor(palette[state % len(palette)])

            if state == self._current_state:
                fill = QColor("#103b52")
                pen = QPen(QColor(COLORS["accent"]), 4)
                label_color = QColor(COLORS["accent"])
...
class GeneFlowApp(QMainWindow):
    ...
    def create_titlebar(self):
        ...
        left_spacer = QWidget()
        left_spacer.setFixedWidth(76)
        layout.addWidget(left_spacer)

        layout.addStretch()

        title = QLabel("GeneFlow Desktop - DNA Matcher")
        title.setFont(QFont(FONTS["mono"], 12, QFont.Weight.Bold))
        title.setStyleSheet(f"color: {COLORS['text-primary']}; letter-spacing: 1px;")
        layout.addWidget(title)

        layout.addStretch()

        right_spacer = QWidget()
        right_spacer.setFixedWidth(76)
    ...
    def create_sidebar(self):
        ...
        def add_section(label_text):
            ...
            lbl.setFont(QFont(FONTS["mono"], 9, QFont.Weight.Bold))
            lbl.setStyleSheet(f"color: {COLORS['text-secondary']}; letter-spacing: 3px; padding: 12px 24px 6px;")
        ...
        line.setStyleSheet(f"background: {COLORS['border']}; margin: 10px 24px;")
    ...
    def _make_nav_button(self, text, kind, active=False):
        ...
        btn.setStyleSheet(
            ...
            QPushButton:hover {{
                background: rgba(209,173,106,0.08);
                color: {COLORS['text-primary']};
                border-left-color: {COLORS['gold']};
            }}
            QPushButton:checked {{
                background: rgba(209,173,106,0.14);
                border-left-color: {COLORS['accent']};
                color: {COLORS['accent']};
            }}
            QPushButton:pressed {{
                background: rgba(209,173,106,0.20);
                color: {COLORS['gold2']};
            }}
            """
        )
    ...
    def _set_theme(self, mode: str):
        ...
        palette = LIGHT_COLORS if mode == "light" else DARK_COLORS
        COLORS.clear()
        COLORS.update(palette)

    def _rebuild_ui_after_theme_change(self):
        saved_pattern = self.pattern_input.text() if hasattr(self, "pattern_input") else self.pattern
        saved_genome = self.genome_input.toPlainText() if hasattr(self, "genome_input") else self.genome
        saved_source = self.genome_source_input.text() if hasattr(self, "genome_source_input") else ""
        ...
        if hasattr(self, "pattern_input"):
            self.pattern_input.setText(saved_pattern)
        if hasattr(self, "genome_input"):
            self.genome_input.setPlainText(saved_genome)
        if hasattr(self, "genome_source_input") and saved_source:
            self.genome_source_input.setText(saved_source)
        if hasattr(self, "config_quick_btn"):
            self.config_quick_btn.setText(f"Theme: {self._theme_mode.title()}")

        if self.pattern and self.genome and self.dfa_table:
    ...
    def create_topbar(self):
        ...
        self.pattern_input.setMaximumWidth(180)
        ...
        self.genome_source_input.setMinimumWidth(120)
        self.genome_source_input.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)

        self.load_btn = QPushButton("Browse")
        self.load_btn.clicked.connect(self.load_fasta)
        self.load_btn.setMinimumWidth(84)
        ...
        label2.setFont(QFont(FONTS["mono"], 10))
        label2.setStyleSheet(f"color: {COLORS['text-tertiary']}; letter-spacing: 2px;")
        label2.setMinimumWidth(120)
        g2.addWidget(label2)
        g2.setStretchFactor(label2, 0)
        g2.addWidget(self.genome_source_input)
        g2.setStretchFactor(self.genome_source_input, 1)
        g2.addWidget(self.load_btn)
        g2.setStretchFactor(self.load_btn, 0)
        layout.addWidget(group2, 1)

        # Let this spacer collapse first on narrow widths to prevent field/button overlap.
        layout.addStretch(1)
    ...
    def _field_style(self, width):
        return f"background: {COLORS['card']}; border: 1px solid {COLORS['border-card']}; border-radius: 6px; padding: 8px 12px; color: {COLORS['text-primary']}; font-family: '{FONTS['mono']}'; font-size: 13px; min-height: 18px;"
    ...
    def _icon_button_style(self):
        return f"""
        background: {COLORS['surface'] if 'surface' in COLORS else COLORS['bg-secondary']};
        color: {COLORS['text-secondary']};
        border: 1px solid {COLORS['border']};
        border-radius: 6px;
        """
    ...
    def create_center_content(self):
        ...
        self.example_btn.setStyleSheet(self._secondary_button_style())
    ...
    def create_metrics_row(self):
        ...
        for label, color in cards:
            ...
            cl.addWidget(lab)
            val = QLabel("—")
            val.setFont(QFont(FONTS["mono"], 28, QFont.Weight.Bold))
    ...
    def create_ai_right_tab(self):
        ...
        self.ai_status = QLabel("Hugging Face AI ready.")
        ...
        if not HAS_AI:
            self.ai_status.setText("AI backend unavailable. Check your Hugging Face token settings.")
    ...
    def create_kpi_strip(self):
        ...
        for key in ["Matches", "Density", "DFA States", "Genome Length"]:
            ...
            lbl.setFont(QFont(FONTS["sans"], 10, QFont.Weight.Bold))
            lbl.setStyleSheet(f"color: {COLORS['text-primary']}; letter-spacing: 0.6px;")
    ...
    def create_ai_chat_tab(self):
        ...
        self.chat_input = QTextEdit()
        self.chat_input.setPlaceholderText("Ask the AI assistant something about your DNA analysis...")
        ...
        self.send_chat_btn = QPushButton("Send to AI")
        ...
        if HAS_AI:
            self.ai_status.setText("Hugging Face AI ready. Uses HUGGINGFACE_API_KEY or HUGGINGFACEHUB_API_TOKEN.")
        else:
            self.ai_status.setText("AI backend unavailable. Check your Hugging Face token settings.")
            self.send_chat_btn.setEnabled(False)
            self.extract_pattern_btn.setEnabled(False)
    ...
    def run_matcher(self):
        ...
        if not pattern:
            QMessageBox.warning(self, "Invalid Pattern", "Enter a DNA pattern using only A, T, C, and G.")
            self.pattern_input.setFocus()
            if self._status_left is not None:
                self._status_left.setText("Invalid pattern - use only A, T, C, G")
                self._status_left.setStyleSheet(
                    f"color: {COLORS['error']}; font-family: '{FONTS['mono']}'; font-size: 10px; font-weight: 700;"
                )
            return

        invalid_chars = sorted({ch for ch in pattern if ch not in {"A", "T", "C", "G"}})
        if invalid_chars:
            bad = ", ".join(invalid_chars)
            QMessageBox.warning(
                self,
                "Invalid Pattern",
                f"Pattern contains invalid characters: {bad}\nUse only A, T, C, and G.",
            )
            self.pattern_input.setFocus()
            if self._status_left is not None:
                self._status_left.setText(f"Invalid pattern characters: {bad}")
                self._status_left.setStyleSheet(
                    f"color: {COLORS['error']}; font-family: '{FONTS['mono']}'; font-size: 10px; font-weight: 700;"
                )
            return
        ...
        if not genome:
            QMessageBox.warning(self, "Missing Genome", "Load or paste a genome sequence before running analysis.")
            if self._status_left is not None:
                self._status_left.setText("Genome sequence missing")
                self._status_left.setStyleSheet(
                    f"color: {COLORS['error']}; font-family: '{FONTS['mono']}'; font-size: 10px; font-weight: 700;"
                )
            return
    ...
    def _ensure_ai_handler(self) -> bool:
        if not HAS_AI:
            QMessageBox.warning(self, "AI Unavailable", "Configure a Hugging Face API token to enable AI chat.")
            return False
        ...
        try:
            ...
            QMessageBox.warning(self, "Hugging Face Setup Error", str(exc))
            self.ai_status.setText("Hugging Face setup failed. Check your token and internet access.")
            return False
    ...
    def _start_ai_worker(self, action: str, message: str):
        if self._ai_thread and self._ai_thread.isRunning():
            self.ai_status.setText("AI is still processing your previous request...")
            return

        self.send_chat_btn.setEnabled(False)
        self.extract_pattern_btn.setEnabled(False)
        self.ai_status.setText("AI is thinking...")
    ...
    def send_chat_message(self):
        ...
        if not message:
            QMessageBox.information(self, "Empty Message", "Type a message before sending to the AI assistant.")
            return
    ...
    def _on_ai_result(self, action: str, success: bool, payload: str):
        ...
        if success:
            ...
            self.ai_status.setText("AI response received.")
        else:
            self._append_chat_message("system", payload)
